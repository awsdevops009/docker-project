# Build: docker build -f node.dockerfile -t dashpradeep/nodeapp .
...
# docker run -d --net=isolated_network --name nodeapp -p 3000:3000 -v $(pwd)/logs:/var/www/logs dashpradeep/nodeapp
...
# docker run -d -p 3000:3000 --link my-mongodb:mongodb --name nodeapp dashpradeep/nodeapp
